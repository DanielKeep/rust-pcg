...
use std::mem::size_of;
use pcg::{PcgEngineTypes, PcgStream};
...
pub const CARD_NUMBERS: &'static [&'static str] = &["A", "2", "3", "4", "5", "6", "7", "8", "9", "T", "J", "Q", "K"];
pub const CARD_SUITS: &'static [&'static str] = &["h", "c", "d", "s"];

#[test]
fn test_pcg32() {
    use pcg::Pcg32 as Rng;

    type RngResult = <Rng as PcgEngineTypes>::Result;
    ...
    // let bits = size_of::<<Rng as PcgEngineTypes>::Result>() * 8;

    // let mut rng = Rng::with_state(42);
    let mut rng = Rng::with_stream(42, 54);
    println!("rng: {:#?}", rng);
    println!(".. internals: {:?}", rng.dump_internals());
    println!(".. is_mcg: {}", <Rng as PcgEngineTypes>::Stream::is_mcg());
    ...
    for (round_i, round) in rounds.iter().enumerate() {
        ...
        // Again.
        rng.backstep(round.numbers.len() as <Rng as PcgEngineTypes>::State);
        assert_eq!(round.dump, &*format!("{:?}", rng.dump_internals()));
        for &ex in round.numbers {
            assert_eq!(ex, rng.next());
        }
        ...
        for ex in round.coins.chars() {
            let got = if rng.next_bounded(2) == 0 { 'T' } else { 'H' };
            assert_eq!(ex, got);
        }
        ...
        for &ex in round.rolls {
            assert_eq!(ex, rng.next_bounded(6) as u32 + 1);
            rolls_used += 1;
        }
        assert_eq!(rolls_used, round.rolls_used);
        assert_eq!(rng_copy.distance_to(&rng).expect("compare rngs for rolls used") as usize, round.rolls_used);

        let mut cards = [0u32; NUM_CARDS];
        for (c, v) in cards.iter_mut().zip(0..NUM_CARDS as u32) { *c = v }
        util::shuffle(&mut cards, &mut rng);

        for (ex, &got_val) in round.cards.split_whitespace().zip(cards.iter()) {
            let got = card_name(got_val);
            assert_eq!(ex, &*got);
        }
    }
}

fn card_name(v: u32) -> String {
    let v = v as usize;
